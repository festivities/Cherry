...
**Audit correction (2026-09-20): AirArmor bypass NOT achieved; Task 2 incomplete; Tasks 3 and 5 partial; M0 NOT achieved.** Earlier completion statements were overstated. External capture is useful evidence, not an anti-tamper bypass. Historical success and current device state must be recorded separately.
...
- **AirArmor enforcement (verified)**: `onAirCallback`@0x2a2b234 (int only) → `SetAirArmorState`@0x200cf9c (write-once latch at +0x4C, only while normal). `IsAirArmorStateNormal`@0x200cf64: abnormal = {1 FAILED_CALLING, 4 ROOTING, 5 SUSPICIOUS_TOOL, 6 REPACKAGED, 9 INSTALLED_TOOL, 10 SUSPICIOUS_FRAMEWORK}; **FRIDA=12, XPOSED=13, DEBUGGER=8 are NORMAL**. Abnormal → ShowAirArmorDetectPopup@0x200cdf0 only (per-frame re-show; button → scene teardown). NO kill/boot/network gate → **detected Frida does not block M0/M1** (but callback 10 co-fires with 12 in practice and its popup can obstruct UI). Detection implementation is inside packed libm0vie.so (entropy 7.8+, invalid JNI_OnLoad bytes; loaded t+0.177s) — not statically extractable; the ptrace self-tracing child (maps only libm0vie) remains unattributed (breakpad is the only proven ptrace user in libgame).
...
- Boot: `SplashScene::setupPatcher` 0x16f3d24 → `ReqCheckConfiguration` 0x1b903b4 → GET `/v4/setInitConf?nationCode=CC&appVer=V` (header `X-Requested-With: LineplayMobileClient`) → `ResCheckConfiguration` 0x1b7cf80 reads `result.{sessionServerInfo,staticDomain,nationCode,isGdprNation,snsLoginUIList[],snsSignUpUIList[]}` (missing→empty, no crash) → msg 10035 → login. checkSession: GET `/v4/checkSession` (cookies cc/AV_AUTH/udid/os/device) → 403+errorCode≠3001 clears cookies → login UI (empty saved auth can skip the request entirely). NO native force-update gate exists; setInitConf→10035 unblocks boot.
...
- Client resource flow: version JSON from game API → per-category manifests → per-file download (CDTFileListIni/CDTMd5VersionListIni, CHttpDownloader, ReqDownloadSessionKey).
...
- Go server: repo root, stdlib only (`main.go`, `http.go`, `cert.go`, `http_test.go`). Listens HTTPS :443 (dual RSA-2048 + ECDSA-256 certs, **explicit CipherSuites incl. static-RSA AES-GCM/CBC** — the client's ancient OpenSSL offers ONLY static-RSA suites; Go 1.22+ default suites fail it; the `tlsrsakex` GODEBUG is REMOVED in Go 1.27 but explicit CipherSuites still works), **HTTP :80** (patcher/item CDN fetches are PLAIN HTTP), TCP observers :10000/:10123. Run via PowerShell Start-Process w/ output redirect.
- **Verified boot flow (runtime)**: goodbye 404 tolerated → lan3rd notice dialog (dismiss "閉じる" at ~760,1461) → intro sequence (Cherry mascot; dress-up fetches `/img/read/arts_item_<cat>_<id>/1409_iteminfo.artsitem` via :80, 404-tolerated) → 「LINEプレイの世界へ！」 button → **setInitConf** → splash resource + checkresource2.json. AirArmor fires DETECTED_FRIDA(12)+SUSPICIOUS_FRAMEWORK(10)+SUSPICIOUS_NETWORK(11) at ~t+60s — game keeps running (verified non-blocking).
- ⚠ **YunDetectService (host, 127.0.0.1:10000) shadows Cherry's gateway observer** — game's gws:10000 connects (23 SYNs/run, waiting for the 15-byte LPN banner that never comes) land on it. Must be resolved before Session B transport work (kill it, or reconfigure).
- Other M0-era findings: notice text = service-termination notice (live lan3rd still serves it); the game auto-skips checkSession when no saved AV_AUTH; `checkresource2.json` 404 = current patcher state (GetContentsVersion error path, tolerated so far).
- Next: Session B (guest/login flow: guest/generate, createSession, sckey.enc, checkresource2.json response, gateway). See `.agents\PLAN.md`.
